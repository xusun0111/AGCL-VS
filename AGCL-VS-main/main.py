...
def main(args):
    ...
    #adj_two_order: [183,183], 379edges
    results = []
    # print('adj_two_order:', adj_two_order, adj_two_order.shape, adj_two_order.nonzero().size())
    # print('adj:', adj, adj.to_dense().nonzero().size())
    ...
    for trial in range(args.ntrials):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.eval_freq == 0:
                cl_model.eval()
                discriminator.eval()
                # print('adj_two_order:', adj_two_order.size(), adj_two_order.nonzero().size())
                adj_1, adj_2, _, _, _ = discriminator(torch.cat((features, str_encodings), 1), edges, adj, adj_two_order)
                # print('adj_1:', adj_1.size(), adj_1.nonzero().size())
                # print('adj_2:', adj_2.size(), adj_2.nonzero().size())
                embedding = cl_model.get_embedding(features, adj_1, adj_2)
